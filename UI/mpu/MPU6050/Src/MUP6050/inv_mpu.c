...
void MPU6050_ReturnTemp1(float *Temperature)
{
	short temp3;
	uint8_t buf[2];
	i2c_read(st.hw->addr, st.reg->temp, 2, buf);
...
#define WindowLong 200
long databuff1[WindowLong+1];
long databuff2[WindowLong+1];
long databuff3[WindowLong+1];
long databuff4[WindowLong+1];


void SimpleLeapWindow(long *AccelInput,long *AccelOutput, uint8_t windowlong)
{
	uint8_t i;
	
	databuff1[0] = AccelInput[0];
	databuff2[0] = AccelInput[1];
	databuff3[0] = AccelInput[2];
	databuff4[0] = AccelInput[3];
	//WindowIndex++;
	databuff1[windowlong] = 0;
	databuff2[windowlong] = 0;
	databuff3[windowlong] = 0;
	databuff4[windowlong] = 0;

	for(i = 0; i < windowlong - 1; i++)
	{
		//移动
		databuff1[windowlong-1-i] = databuff1[windowlong-2-i];
		databuff2[windowlong-1-i] = databuff2[windowlong-2-i];
		databuff3[windowlong-1-i] = databuff3[windowlong-2-i];
		databuff4[windowlong-1-i] = databuff4[windowlong-2-i];
		//累加
		databuff1[windowlong] += databuff1[windowlong-1-i];
		databuff2[windowlong] += databuff2[windowlong-1-i];
		databuff3[windowlong] += databuff3[windowlong-1-i];
		databuff4[windowlong] += databuff4[windowlong-1-i];
	}
	AccelOutput[0] = databuff1[windowlong]/(windowlong - 1);
	AccelOutput[1] = databuff2[windowlong]/(windowlong - 1);
	AccelOutput[2] = databuff3[windowlong]/(windowlong - 1);
	AccelOutput[4] = databuff4[windowlong]/(windowlong - 1);
}
float LastPitch = 0,LastRoll = 0,LastYaw = 0, CurrentPitch = 0, CurrentRoll = 0, CurrentYaw = 0;
...
uint8_t mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	unsigned char more;
	long quat[4]; 
	// gyro陀螺仪 accel加速度  quat四元数
	if(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more))return 1;	 
	
	if(sensors&INV_WXYZ_QUAT) 
	{
	//	SimpleLeapWindow(quat,quat, WindowLong);//滑窗滤波
...
uint32_t dataNum = 0;
uint8_t AngleDataProcessing(float *pitch,float *roll,float *yaw)
{
	float thepitch,theroll,theyaw;
	...
	//	SimpleLeapWindow(quat,quat, WindowLong);//滑窗滤波
		...
		//计算得到俯仰角/横滚角/航向角
			...
			CurrentPitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch
		  CurrentRoll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
		  CurrentYaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
			if(dataNum >= 20)
			{
			D_pitch = CurrentPitch - LastPitch;
			D_roll = CurrentRoll - LastRoll;
			D_yaw = CurrentYaw - LastYaw;
			//
			LastPitch = CurrentPitch ;
			LastRoll  = CurrentRoll ;
			LastYaw   = CurrentYaw ;
			//
			if(fabs(D_pitch) >= 0.05 )
				*pitch = *pitch + D_pitch;
			if(fabs(D_roll) >= 0.05 )
				*roll = *roll + D_roll;
			if(fabs(D_yaw) >= 0.05 )
