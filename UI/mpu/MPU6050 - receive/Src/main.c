...
//		HAL_Delay(1000);
			...
			{
			//	stLoraRxData.u8Crclength = stLoraRxData.u8Alllength - 4;    //除去0xaa,0x55,CRC16
			Uart_Send_Array(stLoraRxData.u8RxBuff, stLoraRxData.u8Alllength);
			}
		}
}
...
void DataInit(void)
{
    ...
    LoraCommCallBackWaitTime = 1200;    // 等待回传时间配置 
